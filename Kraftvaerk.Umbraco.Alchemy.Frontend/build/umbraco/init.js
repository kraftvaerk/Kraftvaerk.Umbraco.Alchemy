...
export default async function initUmbraco(args) {
    ...
    const backendDir = join(rootDir, 'Kraftvaerk.Umbraco.Alchemy.Backend');
    ...
    try {
        ...
        execSync(`dotnet add "${join(projectDir, projectName + '.csproj')}" reference "${join(rootDir, 'Kraftvaerk.Umbraco.Alchemy.Backend', 'Kraftvaerk.Umbraco.Alchemy.Backend.csproj')}"`, { stdio: 'inherit' });
